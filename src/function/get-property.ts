...
const propsFromAllOf = (allOf: ISchema[]): { [name: string]: ISchema } => {
  return allOf.reduce((acc, schema) => {
    return {
      ...acc,
...
export const getInterfaceOrEnumFromSchema = (
  ...
  if (schema.type === "array") {
    ...
    if (!!schema.items && schema.items["$ref"]) {
      const ref = configuration
        .getReference()
        .getImportAndTypeByRef(schema.items["$ref"], path);
  ...
  if (
    ...
    if (isDebug) {
      console.log(
        `Object Schema ${className} (array=${isArray})`,
    ...
    if (
      ...
      if (
        ...
        if (isArray) {
          interfaceProperty.addAdditionalProperties(
            schema.additionalProperties.items.type,
            true
          );
        } else {
          interfaceProperty.addAdditionalProperties(
      ...
      const transformedSchema = propsFromAllOf(schema.allOf);
      return getInterfaceOrEnumFromSchema(
        className,
        originalName,
        { type: "object", properties: transformedSchema },
        path,
        forceDebug
      );
    }
...
const getProperty = (
  ...
  if (isDebug) {
    console.log(
      `Enter get property ${originalName}`,
      JSON.stringify(schema, null, 2)
    );
  }
  if (schema.allOf) {
  ...
  if (schema.type === "array") {
    ...
    if (isPrimitive(schema.type)) {
      return {
        ...
        required:
          (schema.required || []).indexOf(propertyName) > -1 || required,
        description: "",
        isArray: isArray,
  ...
  if (!!enumeration) {
    let newOriginal = `${propertyName}${originalName
      .substring(0, 1)
      .toUpperCase()}${originalName.substring(1)}`;
    ...
    fs.appendFileSync(
      nodePath.join(nestedPath, `${rendered.fileName}.ts`),
      rendered.render
    );
